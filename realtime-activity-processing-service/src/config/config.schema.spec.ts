...
describe('validateConfig', () => {
  ...
  // proves the *behaviour* those gates can't: a fully-populated environment is accepted and the
  // documented defaults (NODE_ENV, PORT, DB_PORT) are actually applied by zod, not just
  ...
  it('accepts a fully-populated environment and applies the documented defaults', () => {
    ...
    expect(result.DB_APP_USERNAME).toBe('rap_app');
  });
  ...
  // against the actual required field this schema declares, DB_APP_PASSWORD. Asserts the real,
  // observable failure behaviour (process.exit(1) called, stderr names the missing field)
  // rather than restating the schema's own source as a string.
