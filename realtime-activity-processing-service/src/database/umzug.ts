 ...
 * loaded. Exported so it's independently testable without a filesystem or a database. Mirrors
 * `promo-code-service/src/database/umzug.ts` verbatim.
 */
 ...
 * portal/back-end's own `src/database/umzug.ts` convention, ARCHITECTURE.md §4). The meta table
 * lives in `realtime_activity_processing`, never `reward_config`/`reward_portal`/`promo_code`
 * (R0/R1) — bootstrapping that schema into existence is the CLI's job (`cli/migrate.ts`), since
 * the meta table's own schema can't exist before the first migration runs on a genuinely fresh
 * database.
 *
 * Migration filenames are numbered (`001_...`, `002_...`) so ordering is deterministic — this
 * service has exactly one agent (`agent-rap-foundation`) authoring this migration chain, so a
  ...
  return new Umzug({
    migrations: {
      ...
      resolve: ({ name, path: migrationPath, context }) => {
        // eslint-disable-next-line @typescript-eslint/no-var-requires -- T-RAP-002: dynamic migration loading
    ...
    storage: new SequelizeStorage({
      sequelize,
      schema: 'realtime_activity_processing',
