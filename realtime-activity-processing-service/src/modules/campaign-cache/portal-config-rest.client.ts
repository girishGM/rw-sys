/**
 * T-INT-011. REST fallback transport for this service's campaign-config client, calling
 * `portal/back-end`'s new REST mirror of `CampaignConfigService` (T-INT-010,
 ...
 * `ARCHITECTURE.md` §1). Ported in *shape*, not code, from RR's own established REST-fallback
 * client (`reward-redemption-service/src/modules/dispatch/reward-tracking-rest.client.ts`,
 * T-RR-035/T-INT-002) — same `@Optional()`-defaulted options object read from `process.env`, same
 * native `fetch` (Node 20, no extra HTTP dependency), same "throw on anything but a real success"
 * contract.
 *
 ...
 * two GET calls and shape their response, exactly mirroring `CampaignConfigClient`'s own two public
 * methods so `campaign-config.client.ts` can branch between the two transports without either one
 ...
 * reads, just a different identification channel: a header instead of a certificate SAN, per that
 * guard's own header comment). Provisioning a real grant row for this identity, and setting the
 * matching `CAMPAIGN_CONFIG_API_TOKEN` value on the portal side, is an operational/deployment
 * concern, not a development one — same precedent `campaign-config.client.ts`'s own header already
 * establishes for gRPC's mTLS material.
 *
 * **304 handling (implementation note 3's polling substitute)**: T-INT-010's `getCampaignConfig`
 * answers a still-current `etag`/`If-None-Match` with a genuine HTTP `304` and an *empty* body
 * (`campaign-config-api.controller.ts`'s own `getCampaignConfig`: `response.status(304); return
 * undefined;`) — but it still sets the `ETag` response header first, unconditionally, before that
 * branch. This client reads that header and synthesizes the same minimal, `notModified: true`
 * `CampaignConfigProto` shape the gRPC transport already returns for its own not-modified case
 * (`campaign-config.client.ts`'s `CampaignConfigProto.notModified` field), so
 * `campaign-config.client.ts`'s own callers never need a transport-specific branch for this case
 * either.
 */
 ...
 * Mirrors the gRPC transport's own not-modified response shape field-for-field (both are the same
 * wire contract, `campaign-config-response.dto.ts`'s own header). */
 ...
 * (`listActiveCampaigns`/`getCampaignConfig`); no REST equivalent of `watchCampaignConfig` exists
 * (implementation note 3 — there is no REST server-streaming push, the ETag-based polling above is
 * the substitute `campaign-config.client.ts` falls through to instead, not built into this class).
 */
@Injectable()
