 ...
 * first — reproduced for real during this task's own retry-1 independent review (the final
 * assertion at this test's own line ~512 saw `status: 'processing'`, a foreign claim, instead of
 * `'pending'`). Same hazard shape, same fix as `claim-worker.spec.ts`'s own two `T-INT-044
 * regression` tests: this test now also acquires `acquireIngestConsumerGroupReaderLease()` (the
 * same cross-process lock every worker-bundle file already takes before starting a real global
 * claim worker) for the whole window its own row sits exposed, closing this specific residual gap
 * without touching production semantics or weakening the assertion itself.
 */
...
describe('RuleEvaluationRowHandler / advisory lock (real Postgres, rap_app role)', () => {
  ...
  // file could otherwise claim it first (reproduced for real in this task's own retry-1 independent
  // review — see this file's own header). Acquires the same cross-process
  // `acquireIngestConsumerGroupReaderLease()` every worker-bundle file already takes before
  // starting a real global claim worker, for the whole exposure window, so no such foreign claim
  // can happen while this test's own row sits `pending`.
  it(
