/**
 * T-008 — `getCampaignTheme` returns ARCHITECTURE.md §5's exact per-campaign icon + gradient for
 * each of the 3 real demo campaign codes, and a safe fallback for anything else. Same assertions
 * as `features/dashboard/campaignTheme.test.ts` (T-007) since this module is a deliberate,
 * documented duplicate of that one — see this feature's `campaignTheme.ts` header.
 */
