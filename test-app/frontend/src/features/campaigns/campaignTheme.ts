/**
 * T-008 — per-campaign brand color + icon (`ARCHITECTURE.md` §5), used consistently everywhere a
 * campaign appears: the Dashboard's running-campaigns chip/tracker icon (T-007's own copy of this
 * module) and this feature's `/campaigns` list chip + Campaign Detail hero banner. Values are
 * copied verbatim from `ARCHITECTURE.md` §5's own per-campaign table rather than derived — that
 * section is explicit that "the same campaign reads as the same color everywhere", so this module
 * reproduces the exact same gradient stops from the same source doc rather than importing
 * `features/dashboard/campaignTheme.ts`, since `features/dashboard/**` and `features/campaigns/**`
 * are two different tasks' owned file scopes (`AGENT-PROTOCOL.md` R3) and neither owns a shared
 * cross-feature module to put this in (see that file's own header for the same reasoning, written
 * first).
 */
