...
describe('TemporaryPasswordReveal', () => {
  it('hides the password until Reveal is clicked (BACKLOG B-01 / T-034 implementation note 5)', async () => {
  ...
  it('names the tenant admin, not a generic account', () => {
    render(
      <TemporaryPasswordReveal email="admin@example.invalid" temporaryPassword="s3cr3t!Value" />,
    );
    expect(
      screen.getByText(/Tenant Admin account created for admin@example\.invalid/),
    ).toBeInTheDocument();
  });
});
