/**
 * T-030 — a narrow, self-contained rendering of BACKLOG.md B-01's contract for the one screen
 * this task owns (Country Admin onboarding): *"the temporary password is shown once on screen
 * ... hidden behind an explicit Reveal click ... copy-to-clipboard ... never shown again."*
 *
 * This is deliberately **not** the general-purpose panel — that is T-046's own scope
 * (`front-end/src/features/users/PasswordRevealPanel.tsx`), which T-030 cannot depend on (its
 * `Depends on` list is Wave-2-only, and T-046 depends on T-035, which depends on T-030's own
 * T-034). T-046 is expected to fold this call site into its own generic component once it lands;
 * noted for the reviewer, mirroring the note in `country-admin-password.ts`.
 *
  ...
  return (
    <div className="flex flex-col gap-3 rounded-card border border-warning-200 bg-warning-50 p-4">
      <p className="text-sm font-medium text-warning-900">
        Country Admin account created for {email}
      </p>
