...
const { mockMutateAsync, mockUseCreateCountryAdminMutation } = vi.hoisted(() => ({
  mockMutateAsync: vi.fn(),
  mockUseCreateCountryAdminMutation: vi.fn(),
}));

vi.mock('./api', () => ({ useCreateCountryAdminMutation: mockUseCreateCountryAdminMutation }));
...
function renderModal(onClose = vi.fn()) {
  ...
  return render(
    <QueryClientProvider client={client}>
      <AddCountryAdminModal open onClose={onClose} countryId={1} />
    </QueryClientProvider>,
  );
}
...
beforeEach(() => {
  ...
  mockUseCreateCountryAdminMutation.mockReset();
  mockUseCreateCountryAdminMutation.mockReturnValue({
    mutateAsync: mockMutateAsync,
    isPending: false,
  });
});

describe('AddCountryAdminModal', () => {
  ...
  it('shows the server message on an ApiError failure (e.g. duplicate email)', async () => {
    mockMutateAsync.mockRejectedValue(
      new ApiError({ code: 'ADMIN_EMAIL_EXISTS', message: 'Already in use.', status: 409 }),
    );
    const user = userEvent.setup();
    renderModal();
