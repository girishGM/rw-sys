 ...
 *
 * Bootstraps `CREATE SCHEMA IF NOT EXISTS reward_redemption` before constructing Umzug's own
 * `SequelizeStorage`, because that storage's meta table lives IN `reward_redemption` — on a
 ...
 * with `001_create_schema`'s own `up()` (which is what actually gets recorded as "applied") —
 * both are `IF NOT EXISTS`, so running it twice is free. Mirrors RAP's own
 * `src/database/cli/migrate.ts` verbatim in shape (T-RR-002 note 1).
 */
/* eslint-disable no-console -- T-RR-002: this is a CLI script; printing status IS its job. */
 ...
 * loading (once T-RR-004's config module exists) never runs here, so the same precedence is
 * replicated by hand: `.env.local` > `.env.<NODE_ENV>` > `.env`, first one found wins (dotenv
 * never overwrites a variable already loaded).
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await sequelize.query('CREATE SCHEMA IF NOT EXISTS reward_redemption;', {
      type: QueryTypes.RAW,
    });
    ...
    if (command === 'down') {
      ...
      // table lives IN reward_redemption.migrations, so dropping the schema mid-chain breaks
      ...
      if (rollbackAll) {
        await sequelize.query('DROP SCHEMA IF EXISTS reward_redemption CASCADE;', {
          type: QueryTypes.RAW,
        });
        console.log('  ✓ Dropped reward_redemption schema (--all)\n');
      } else {
        console.log();
      }
      return;
