/**
 * T-RR-011. Builds the `@nestjs/microservices` `MicroserviceOptions` for the mTLS
 * `RewardIngestService` gRPC transport (`03-GRPC-CONTRACT.md` §1) from `grpc-server.config.ts`'s
 ...
 * `agent-rr-foundation`) can call it identically.
 *
 * `Transport.GRPC` + `@grpc/proto-loader`, not a hand-rolled `grpc.Server` — same tech-stack choice
 * `ARCHITECTURE.md` §4 makes for this whole service. Same shape as RAP's own
 * `grpc-server.bootstrap.ts` (confirmed by direct read).
 */
...
export function buildGrpcMicroserviceOptions(): GrpcOptions | null {
  ...
  const credentials = ServerCredentials.createSsl(
    ...
    // handshake — `MtlsGuard` is what rejects those, one layer up (TC-6).
    true,
  );
