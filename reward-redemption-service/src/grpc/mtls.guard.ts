/**
 * T-RR-011. mTLS + allowlist enforcement for `RewardIngestService.SubmitRewardEntry`
 * (`03-GRPC-CONTRACT.md` §1, TC-5/TC-6, implementation note 5). Same structure as RAP's own
 * inbound `ActivityIngestService` guard
 * (`realtime-activity-processing-service/src/grpc/mtls.guard.ts`, confirmed by direct read) — the
 * TLS layer itself (`grpc.ServerCredentials.createSsl(..., checkClientCertificate: true)`, wired
 * in `grpc-server.bootstrap.ts`) already refuses any connection that doesn't present a certificate
 * signed by the configured CA, before this guard, or any handler, ever runs. This guard's own job
 * is narrower and runs *after* a TLS-valid connection is established: is this specific
 * certificate's identity on this service's own allowlist, and if so, which `tenantId` does it
 * resolve to (`service-identity.registry.ts`'s own header — this service's allowlist is `identity
 * -> tenantId`, not just `identity -> allowed/denied`, since `RewardEntry` has no `tenant_id` field
 * of its own whose trustworthiness could otherwise be verified).
 *
 ...
 * token or cookie header (a different trust domain from every REST endpoint this plan builds).
 *
 ...
 * positional args, so `context.getArgByIndex(2)` is the raw `grpc.ServerUnaryCall`.
 */
