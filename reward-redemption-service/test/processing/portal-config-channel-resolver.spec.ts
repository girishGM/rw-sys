/**
 * T-INT-012 — `PortalConfigChannelResolverService`, unit-tested against a fake `pg.Pool`
 * substitute (mirroring `FIND_ONE_SQL`'s own exact-triple lookup in plain JS), an explicit direct
 * port of RAP's own identical suite
 * (`realtime-activity-processing-service/test/campaign-cache/portal-config-channel-resolver.spec.ts`,
 * T-INT-011 — confirmed by direct read before diverging in shape), same "the precedence/caching
 ...
 * migration `025`) is exercised for real as part of this task's own Verification steps 2-3
...
} from '@/modules/processing/portal-config-channel-resolver.service';
...
describe('T-INT-012 — PortalConfigChannelResolverService', () => {
