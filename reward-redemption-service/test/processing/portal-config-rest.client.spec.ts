/**
 * T-INT-012 — `PortalConfigRestClient` against a mocked global `fetch` (Node 20's own built-in
 * implementation, no extra HTTP-client dependency added), an explicit direct port of RAP's own
 * identical suite
 * (`realtime-activity-processing-service/test/campaign-cache/portal-config-rest.client.spec.ts`,
 * T-INT-011 — confirmed by direct read before diverging in shape), the same deterministic, no-real-
 * network pattern `reward-tracking-rest.client.spec.ts` (T-RR-035/T-INT-002) already established
 * for the equivalent REST-fallback client in this service. Asserts the *outgoing request* this
 * client actually builds (URL, headers, query string) against portal's real, shipped contract
 * (`campaign-config-api.controller.ts`) — `AGENT-PROTOCOL.md` §3's "assert the observable property"
 * discipline — not just a change-detector on an internal constant. Uses `CAMPAIGN_CONFIG_SECTIONS`
 * throughout (never `RULES` — this service's own `ConfigSectionName` type doesn't include it at
 * all, `campaign-config.client.ts`'s own header, implementation note 1 of the T-RR-022 task file).
 */
...
import {
  ...
  loadPortalRestToken,
} from '@/modules/processing/portal-config-rest.client';
...
describe('T-INT-012 — PortalConfigRestClient', () => {
  ...
  function buildClient(
    ...
    return new PortalConfigRestClient({
      ...
      serviceIdentity: 'reward-redemption-service',
      timeoutMs: 1000,
      ...overrides,
    });
  }
  ...
  describe('loadPortalRestToken / loadPortalRestServiceIdentity / loadPortalConfigRestClientOptions', () => {
    ...
    it('falls back to documented defaults for base URL/timeout when unset', () => {
      ...
      process.env.PORTAL_REST_SERVICE_IDENTITY = 'rr';
  ...
  describe('listActiveCampaigns', () => {
    it('GETs the real T-INT-010 route with the required auth headers', async () => {
      ...
      const client = buildClient({ token: 'super-secret', serviceIdentity: 'rr-identity' });
      const result = await client.listActiveCampaigns(7, ['BASIC', 'MERCHANTS']);

      expect(result.campaigns).toEqual(campaigns);
      expect(fetchSpy).toHaveBeenCalledTimes(1);
      const [url, init] = fetchSpy.mock.calls[0];
      expect(url).toBe(
        'http://portal.test/api/v1/campaign-config/tenants/7/campaigns?sections=BASIC,MERCHANTS',
      );
      expect(init.method).toBe('GET');
      expect(init.headers).toMatchObject({
        Authorization: 'Bearer super-secret',
        'X-Service-Identity': 'rr-identity',
      });
    });
    ...
    it('never requests the RULES section — this service has no such section at all', async () => {
      fetchSpy.mockResolvedValue(
        fakeResponse(200, {
          data: { campaigns: [], servedAt: 'now', sectionsReturned: [], sectionsOmitted: [] },
        }),
      ...
      expect(url).not.toContain('RULES');
      expect(url).toContain('sections=BASIC,MERCHANTS,TRACKERS,REWARDS,CAPS');
    });
  ...
  describe('getCampaignConfig', () => {
    it('GETs the real T-INT-010 route, forwarding sections and etag as query params', async () => {
      ...
      const result = await client.getCampaignConfig(9, 'CMP-9', ['CAPS'], 'prior-etag');

      expect(result).toEqual(campaign);
      const [url] = fetchSpy.mock.calls[0];
      expect(url).toBe(
        'http://portal.test/api/v1/campaign-config/tenants/9/campaigns/CMP-9?sections=CAPS&etag=prior-etag',
      );
    });
