/**
 * T-RR-005. Pure, synchronous unit tests — no network, no DB. TC-5/TC-6's own "process exits
 * non-zero at boot" observable is proven separately, against a real spawned subprocess
 * (`boot.e2e-spec.ts`), since a mocked `process.exit` here would only prove this function *calls*
 * something, not that a real process actually terminates non-zero.
 */
import {
  EncryptionService,
...
describe('loadEncryptionKeyMaterial', () => {
  ...
  // TC-5 (the in-process half — the real "exits non-zero" half is boot.e2e-spec.ts).
...
describe('EncryptionService', () => {
  ...
  it('rejects a ciphertext too short to contain an IV and an auth tag', () => {
    expect(() => service.decrypt(Buffer.from('short').toString('base64'))).toThrow(/too short/);
  });
  ...
  // TC-4
  it('hashes two different real-looking customerId values to different results', () => {
