/**
 * T-RR-011. Fast, mocked-dependency unit tests for `MtlsGuard` — the real-mTLS/real-Postgres round
 * trip (TC-5/TC-6/TC-7) lives in `reward-ingest.e2e-spec.ts` instead, same split
 * `activity-ingest.controller.spec.ts`/`grpc-server.e2e-spec.ts` (T-RAP-022) already established
...
describe('T-RR-011 — MtlsGuard (unit, mocked registry)', () => {
  ...
  // TC-6: wrong/missing service identity is rejected before the domain method is ever reached.
...
describe('T-RR-011 — ServiceIdentityRegistry (unit)', () => {
