/**
 * T-RR-011 test support. Generates an ephemeral CA + server certificate + per-identity client
 * certificates via the system `openssl` binary into a fresh temp directory per test run. Nothing
 * here is committed material (`AGENT-PROTOCOL.md` R1 — no secret in a committed file): every key is
 * generated fresh, used only for the lifetime of one `describe` block, and the temp directory is
 * deleted in `afterAll`. Same precedent RAP's own
 * `realtime-activity-processing-service/test/grpc/support/test-cert-authority.ts` (T-RAP-022,
 * confirmed by direct read) already set for the sibling project.
 *
...
export class TestCertAuthority {
  ...
  static build(): TestCertAuthority {
    const dir = mkdtempSync(join(tmpdir(), 'rr-grpc-mtls-'));
