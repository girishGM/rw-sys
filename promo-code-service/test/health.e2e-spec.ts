...
describe('GET /health (e2e)', () => {
  ...
  // body." Boots the real `AppModule` (proving `ConfigModule` + `HealthModule` wiring, not
  // just the controller in isolation) against `DB_HOST`/`DB_PORT` from
  // `test/jest-e2e.setup.ts`, which point at the real Postgres 16 server documented in root
  // CLAUDE.md — this asserts the actual outcome a TCP client observes against that real
  // server, not a mocked/stubbed reachability result.
  ...
  // proves the /health endpoint's DB probe is a real, falsifiable check, not one that
  // reports "ok" unconditionally. Port 1 is a well-known reserved (unassignable) port, never
  // accepting TCP connections on any dev machine.
  //
  // Builds a minimal module with `HealthController` and a stand-in `ConfigService` as
  // SIBLING providers (not the whole `AppModule`/`ConfigModule`) rather than mutating
  ...
  // Declaring the controller directly here (rather than importing `HealthModule`, which
  // relies on `ConfigService` being registered globally by `AppModule` in production) keeps
  // this stand-in in the same module as the controller that consumes it — Nest only resolves
  // a dependency from providers visible to the requesting module, not from a sibling test
